...
public class ItemServiceTest {
    ...
    List<MultipartFile> createMultipartFiles() throws Exception { // MockMultipartFile 클래스를 이용하여 가짜 MultipartFile ㅣㄹ스트를 만들어서 반환

        List<MultipartFile> multipartFileList = new ArrayList<>();

        for (int i = 0; i < 5; i++) {
            String path = "D:/upload/shop/item/";
    ...
    void saveItem() throws Exception{
        ...
        assertEquals(multipartFileList.get(1).getOriginalFilename(), itemImgList.get(0).getOriImgName()); // 상품 이미지는 첫 번째 파일의 원본 이미지 파일 이름만 같은지 확인
